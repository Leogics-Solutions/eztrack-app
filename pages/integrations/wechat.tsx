...
export default function WeChatIntegrationsPage() {
  ...
  useEffect(() => { setLoading(true); void load(); }, [load, selectedOrganizationId]);
...
function ConnectionCard({ connection, groups, query, busy, setQuery, onRefresh, onConnect, onDisconnect, onGroups, onTest, onDelete }: { connection: WeChatConnection; groups?: WeChatGroup[]; query: string; busy: string | null; setQuery: (value: string) => void; onRefresh: () => void; onConnect: () => void; onDisconnect: () => void; onGroups: () => void; onTest: (group: WeChatGroup) => void; onDelete: () => void }) {
  ...
  const runnerConnected = connection.status === 'connected';
  const listenerReady = runnerConnected && connection.message_listener_ready;
  return <article className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><MessageCircle className="h-5 w-5 text-cyan-700" /><h2 className="font-semibold">{connection.name}</h2><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${runnerConnected ? 'bg-emerald-100 text-emerald-900' : 'bg-amber-100 text-amber-950'}`}>{connection.status}</span></div><p className="mt-1 text-xs text-[var(--muted-foreground)]">{connection.display_name || connection.account_id || `Connection #${connection.id}`} {connection.client_version ? `· client ${connection.client_version}` : ''}</p></div><div className="flex flex-wrap gap-2">{!runnerConnected ? <button onClick={onConnect} disabled={Boolean(busy)} className="rounded-lg bg-cyan-700 px-3 py-2 text-xs font-semibold text-white">Connect</button> : <>{!listenerReady && connection.group_ids.length > 0 && <button onClick={onConnect} disabled={Boolean(busy)} className="rounded-lg bg-cyan-700 px-3 py-2 text-xs font-semibold text-white">Restore listener</button>}<button onClick={onDisconnect} disabled={Boolean(busy)} className="inline-flex items-center gap-1 rounded-lg border border-[var(--border)] px-3 py-2 text-xs font-semibold"><Unplug className="h-3.5 w-3.5" /> Disconnect</button></>}<button onClick={onRefresh} disabled={Boolean(busy)} className="rounded-lg border border-[var(--border)] p-2"><RefreshCw className={`h-4 w-4 ${busy ? 'animate-spin' : ''}`} /></button><button onClick={onDelete} disabled={Boolean(busy)} className="rounded-lg border border-red-300 p-2 text-red-700"><Trash2 className="h-4 w-4" /></button></div></div>{connection.last_error && <p className="mt-3 rounded-lg bg-red-50 p-3 text-sm text-red-900">{connection.last_error}</p>}
    <div className="mt-4"><button onClick={onGroups} disabled={!runnerConnected || Boolean(busy)} aria-describedby={!runnerConnected ? `groups-help-${connection.id}` : undefined} title={!runnerConnected ? 'Connect the local WeChat runner before loading groups.' : undefined} className="inline-flex items-center gap-2 rounded-lg border border-[var(--border)] px-3 py-2 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50">{busy === `groups-${connection.id}` ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Users className="h-4 w-4" />} View groups</button>{!runnerConnected && <p id={`groups-help-${connection.id}`} className="mt-2 text-xs font-medium text-amber-800 dark:text-amber-200">Connect the local WeChat runner before loading groups.</p>}{groups && <><label className="relative mt-3 block"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted-foreground)]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search group name or ID" className="w-full rounded-lg border border-[var(--border)] bg-[var(--background)] py-2.5 pl-9 pr-3 text-sm" /></label><div className="mt-2 max-h-72 overflow-y-auto rounded-lg border border-[var(--border)]">{visible.map((group) => <div key={group.id} className="flex items-center justify-between gap-3 border-b border-[var(--border)] px-3 py-2.5 last:border-0"><div><p className="text-sm font-medium">{group.name}</p><p className="text-xs text-[var(--muted-foreground)]">{group.member_count} members · {group.id}</p></div><button onClick={() => onTest(group)} className="inline-flex items-center gap-1 rounded-md border border-[var(--border)] px-2.5 py-1.5 text-xs font-semibold"><Send className="h-3.5 w-3.5" /> Test</button></div>)}{!visible.length && <p className="p-5 text-center text-sm text-[var(--muted-foreground)]">No matching groups.</p>}</div></>}</div>
    {runnerConnected && <p className={`mt-3 rounded-lg p-3 text-sm font-semibold ${listenerReady ? 'bg-emerald-50 text-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-200' : 'bg-amber-50 text-amber-950 dark:bg-amber-950/30 dark:text-amber-200'}`}>{listenerReady ? `Listening to ${connection.group_ids.length} configured group${connection.group_ids.length === 1 ? '' : 's'}.` : connection.group_ids.length > 0 ? 'Connected, but the message listener is not ready. Click Restore listener.' : 'Runner connected. No monitored groups are configured yet; load groups here, then map them in the Payment Knock-Off automation.'}</p>}
  </article>;
}
